...
for(lbl in names(datasets_as_matrix)) {
  ...
  for(alg_norm in msdap::normalization_algorithms()) {
    ...
    m_log2_norm_1 = msdap::normalize_matrix(m_log2, algorithm = alg_norm, mask_sample_groups = m_group_id)
    testthat::test_that(paste("dataset:", lbl, "norm:", alg_norm), {
      for(i in 1:3) {
        # cat(" ", i)
        rng = rnorm(1)
        m_log2_norm_2 = msdap::normalize_matrix(m_log2, algorithm = alg_norm, mask_sample_groups = m_group_id)
