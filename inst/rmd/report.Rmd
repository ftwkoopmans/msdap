...
For each sample group, two plots are shown: 1) a basic monochrome plot and 2) a variant that color-codes the top10 'worst' samples based on the standard deviation of their respective distributions. The legend is sorted in column-first descending order (sample with highest sd in column 1 row 1, sample with second highest sd in column 1 row 2, etc.). If there are 10 or fewer samples, all are color-coded. 
...
if(isplot_cov) {
  ...
  cat("The user-specified filtering rules (eg; filter_min_detect, filter_min_peptide_per_prot, etc.) were applied within each sample group independently and remaining peptides were subsequently normalized. ")
...
p_cov = list()
if(isplot_cov) {
  x = rollup_pep2prot(dataset$peptides %>% select(peptide_id, protein_id, sample_id, intensity=intensity_by_group), algo_rollup = "maxlfq")
...
if(isplot_cov_protein) {
  cat('\n\\newpage \n')
  cat("**Protein-level CoV:** (analogous to peptide CoV's, but with additional MaxLFQ rollup to protein abundances) \n\n")
...
```{r, echo=F, message=F, warning=F, results="asis"} 
if(exists("tib_report_stats_summary") && nrow(tib_report_stats_summary) > 0) {
  ...
  if(exists("tib_report_diffdetects_summary") && nrow(tib_report_diffdetects_summary) > 0) {
