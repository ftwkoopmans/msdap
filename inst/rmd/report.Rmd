...
subtitle: 'version: beta 0.2.2  &nbsp; &nbsp;  https://github.com/ftwkoopmans/msdap/'
output:
  pdf_document:
    toc: true
    toc_depth: 2
...
# setting crop=NULL prevents PDF cropping, while it desirable in many cases it produces problems we cannot currently resolve on unix systems
knitr::opts_chunk$set(echo = FALSE, dpi=300, fig.width=7, fig.height=3.5, fig.align="center", tidy.opts=list(width.cutoff=80), tidy=TRUE, crop = NULL)
```
...
samples_metadata_counts_colors = samples_colors_long %>%
  ...
  left_join(dataset$samples %>% select(shortname, detected_peptides), by = "shortname") %>%
...
for(prp in names(l)) {
  subchunkify(print(l[[prp]]$plot), unique_chunk_id = paste0("detect_vs_metadata_scatterplot_", prp), fig_height = min(10, 1.5 + 0.2 * l[[prp]]$n), fig_width = 7)
  cat('\n\n')
}
```
...
print(p_intensity$intensity_distributions_all)
cat('\n\n')
...
if(length(l_contrast) > 0) {
  ...
  for(contr in names(l_contrast)) {
    ...
    tib_log = dataset$peptides %>% filter(!is.na(!!col_contr_intensity)) %>% summarise(n_precursor=n_distinct(peptide_id), n_prot=n_distinct(protein_id))
    ...
    i=0
    for(p in l_contrast[[contr]]$p_volcano_contrast) {
      subchunkify(print(p), unique_chunk_id = paste0("volcano_", contr, "_", i), fig_height = 8, fig_width = 7)
      cat('\n\n') 
      i = i + 1
    }
    ...
    subchunkify(print(l_contrast[[contr]]$p_foldchange_density), unique_chunk_id = paste0("foldchange_density_", contr), fig_height = 7, fig_width = 7)
    ...
    subchunkify(print(l_contrast[[contr]]$p_pvalue_hist), unique_chunk_id = paste0("pvalue_hist_", contr), fig_height = 7, fig_width = 7)
    ...
    if(exists("dd_plots") && contr %in% names(dd_plots)) {
      cat('\n\\newpage \n')
      cat('\n### differential detect \n\n')
      cat('Some proteins may not have peptides with sufficient abundance values for differential expression analysis, but do show strong differences in the number of detected peptides between sample groups. A simple z-score is computed for each protein based on the number of detected peptides per sample group. DEA is much more powerful for differential testing, but theze z-scores can offer complementary information in some experimental designs (e.g. a wildtype-knockout APMS study). \n\n')
      cat('Below figure shows the distribution of these scores with thresholds at 2 std. Both the z-scores and the counts these are based upon are available in the statistical result Excel table. \n\n')
      
      subchunkify(print(dd_plots[[contr]]), unique_chunk_id = paste0("dd_hist_", contr), fig_height = 4, fig_width = 4)
      # cat('\n\n **top10 candidate genes:** \n\n')
      # cat(paste(head(dd_plots[[contr]]$genes, 10), collapse = ", "), "\n")
    }
...
formatted_code = tryCatch(subset_relevant_code_snippet_for_report(format_r_code(history_as_string)), error = function(e) {""})
cat('```r \n')
