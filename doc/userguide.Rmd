...
## EncyclopeDIA
No particular settings are needed, MS-DAP can use the output as-is. The .elib files generated by EncyclopeDIA for each input sample (mzML file) are the input for MS-DAP.

MS-DAP import function: `msdap::import_dataset_encyclopedia()`


## Spectronaut
No particular settings are needed, but do note that a report with peptide abundance values should be prepared that;

1) has no filters enabled (don't remove any peptide and keep decoys in the output)
2) contains these columns: R.FileName, PG.ProteinGroups, EG.IsDecoy, EG.Library, EG.Qvalue, EG.iRTEmpirical, EG.Cscore, FG.Id, FG.MS2Quantity, FG.MS2RawQuantity
